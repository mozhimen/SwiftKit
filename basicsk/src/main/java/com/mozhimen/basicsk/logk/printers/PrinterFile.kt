 ...
 * @param _retentionTime log文件的有效时长，单位毫秒，<=0表示一直有效
 * @Author Kolin Zhao / Mozhimen
 * @Date 2021/12/20 17:33
 * @Version 1.0
 */
class PrinterFile(
    private val _logPath: String,
    ...
    companion object {
        ...
        @Synchronized
        fun getInstance(
            logPath: String? = UtilKGlobal.instance.getApp()!!.cacheDir.absolutePath,
            retentionTime: Long? = 0
        ): PrinterFile {
            if (instance == null) {
                instance = PrinterFile(logPath!!, retentionTime!!)
            }
            return instance!!
        }
    }
    ...
    private inner class PrinterWriter {
        ...
        fun ready(newFileName: String): Boolean {
            ...
            if (!_logFile!!.exists()) {
                try {
                    val parent = _logFile!!.parentFile
                    if (!parent.exists()) {
                        parent.mkdirs()
                    }
