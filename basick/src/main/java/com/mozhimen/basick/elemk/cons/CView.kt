...
object CView {
    object SystemUi {
        ...
        @ADescription("稳定的布局, 不会随系统栏的隐藏/显示而变化", "须FLAG_LAYOUT_FULLSCREEN", "FLAG_LAYOUT_HIDE_NAVIGATION", "android:fitsSystemWindows要设置为true")
        const val FLAG_LAYOUT_STABLE = View.SYSTEM_UI_FLAG_LAYOUT_STABLE

        //拓展布局到导航栏后面-->
        // 将布局内容拓展到导航栏和状态栏的后面
        @ADescription("沉浸模式, 将布局内容拓展到导航栏和状态栏的后面")
        const val FLAG_LAYOUT_HIDE_NAVIGATION = View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION

        //拓展布局到状态栏后面-->
        // 将布局内容拓展到状态栏的后面
        @ADescription("沉浸模式, 将布局内容拓展到状态栏的后面")
        ...
        // 所以, FLAG_IMMERSIVE都是配合FLAG_FULLSCREEN和FLAG_HIDE_NAVIGATION一起使用的
        // 在没有设置FLAG_IMMERSIVE时, 随便点击屏幕就可以解除隐藏导航栏的状态.所以, 设置FLAG_IMMERSIVE就是真正进入沉浸模式, 一般在
        @ADescription("全屏->沉浸模式, 点击屏幕的任意位置, 就会恢复为正常模式")
        ...
        @ADescription("全屏->沉浸->全屏模式, 用户上下拉系统栏时, 会自动隐藏系统栏")
