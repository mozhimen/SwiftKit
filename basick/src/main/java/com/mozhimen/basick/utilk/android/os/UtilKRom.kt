...
object UtilKRom {
    ...
    fun checkRom(romStr: String): Boolean {
        ...
        if (!TextUtils.isEmpty(UtilKProcess.getProp(CPackage.RO_MIUI_UI_VERSION_NAME).also { _version = it })) {
            _name = CRom.MIUI
        } else if (!TextUtils.isEmpty(UtilKProcess.getProp(CPackage.RO_BUILD_VERSION_EMUI).also { _version = it })) {
            _name = CRom.EMUI
        } else if (!TextUtils.isEmpty(UtilKProcess.getProp(CPackage.RO_BUILD_VERSION_OPPOROM).also { _version = it })) {
            _name = CRom.OPPO
        } else if (!TextUtils.isEmpty(UtilKProcess.getProp(CPackage.RO_VIVO_OS_VERSION).also { _version = it })) {
            _name = CRom.VIVO
        } else if (!TextUtils.isEmpty(UtilKProcess.getProp(CPackage.RO_SMARTISAN_VERSION).also { _version = it })) {
