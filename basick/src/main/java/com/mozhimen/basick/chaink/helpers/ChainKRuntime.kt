 ...
 * @Version 1.0
 */
internal object ChainKRuntime : BaseUtilK(), IChainKRuntime {
     ...
     */
    private val _waitingTasks: MutableList<BaseChainKTask> = mutableListOf()

    /**
     * 记录下启动阶段所有任务的运行时信息key是taskId
     */
    private val _chainkTaskRuntimeInfos: MutableMap<String, MChainKTaskRuntimeInfo> = HashMap()
    ...
    override fun setThreadName(task: BaseChainKTask, threadName: String) {
        val taskKRuntimeInfo = getTaskRuntimeInfo(task.id)
        taskKRuntimeInfo?.threadName = threadName
    }

    override fun setTaskStateInfo(task: BaseChainKTask) {
        val taskKRuntimeInfo = getTaskRuntimeInfo(task.id)
        taskKRuntimeInfo?.setStateTime(task.state, System.currentTimeMillis())
    }

    override fun getTaskRuntimeInfo(id: String): MChainKTaskRuntimeInfo? {
    ...
    override fun executeTask(task: BaseChainKTask) {
        if (task.isAsyncTask) {
    ...
    override fun traversalDependencyTreeAndInit(task: BaseChainKTask) {
        val traversalVisitor = linkedSetOf<BaseChainKTask>()
    ...
    private fun addWaitingTask(task: BaseChainKTask) {
    ...
    private fun hasBlockBehindTask(task: BaseChainKTask): Boolean {
        if (task is CriticalChainKTask) return false
    ...
    private fun traversalDependencyPriority(task: BaseChainKTask?) {
        if (task == null) return
    ...
    private fun innerTraversalDependencyTreeAndInit(task: BaseChainKTask, traversalVisitor: LinkedHashSet<BaseChainKTask>) {
        ////物始化taskKRuntimeInfo 并校验是否存在相同的任务名称taskK.Id
        var taskKRuntimeInfo = getTaskRuntimeInfo(task.id)
        if (taskKRuntimeInfo == null) {
            taskKRuntimeInfo = MChainKTaskRuntimeInfo(task)
