...
/**
 * 标签View
 * 参考 https://github.com/donkingliang/LabelsView
 */
public class LayoutKLabelGroup extends ViewGroup implements View.OnClickListener, View.OnLongClickListener {
    ...
    private int mLabelColumnCount = -1;
    ...
    public LayoutKLabelGroup(Context context) {
        super(context);
        mContext = context;
        showEditPreview();
    }
    public LayoutKLabelGroup(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        getAttrs(context, attrs);
        showEditPreview();
    }
    public LayoutKLabelGroup(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void getAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            TypedArray mTypedArray = context.obtainStyledAttributes(attrs, R.styleable.LayoutKLabelGroup);
            int type = mTypedArray.getInt(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_selectType, 1);
            mSelectType = SelectType.get(type);

            mMaxSelect = mTypedArray.getInteger(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_maxSelect, 0);
            mMinSelect = mTypedArray.getInteger(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_minSelect, 0);
            mMaxLines = mTypedArray.getInteger(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_maxLines, 0);
            isIndicator = mTypedArray.getBoolean(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_isIndicator, false);

            mLabelGravity = mTypedArray.getInt(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_labelGravity, mLabelGravity);
            mLabelColumnCount = mTypedArray.getInt(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_labelColumnCount, mLabelColumnCount);
            if (mLabelColumnCount <= 0) {
                mLabelWidth = mTypedArray.getLayoutDimension(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_labelTextWidth, mLabelWidth);
            } else {
                mLabelWidth = LayoutParams.MATCH_PARENT;
            }
            mLabelHeight = mTypedArray.getLayoutDimension(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_labelTextHeight, mLabelHeight);

            if (mTypedArray.hasValue(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_labelTextColor)) {
                mTextColor = mTypedArray.getColorStateList(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_labelTextColor);
            } else {
            ...
            if (mTypedArray.hasValue(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_labelTextPadding)) {
                ...
                mTextPaddingLeft = mTypedArray.getDimensionPixelOffset(
                        R.styleable.LayoutKLabelGroup_layoutKLabelGroup_labelTextPaddingLeft, dp2px(10));
                mTextPaddingTop = mTypedArray.getDimensionPixelOffset(
                        R.styleable.LayoutKLabelGroup_layoutKLabelGroup_labelTextPaddingTop, dp2px(5));
                mTextPaddingRight = mTypedArray.getDimensionPixelOffset(
                        R.styleable.LayoutKLabelGroup_layoutKLabelGroup_labelTextPaddingRight, dp2px(10));
                mTextPaddingBottom = mTypedArray.getDimensionPixelOffset(
                        R.styleable.LayoutKLabelGroup_layoutKLabelGroup_labelTextPaddingBottom, dp2px(5));
            }

            mLineMargin = mTypedArray.getDimensionPixelOffset(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_lineMargin, dp2px(5));
            mWordMargin = mTypedArray.getDimensionPixelOffset(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_wordMargin, dp2px(5));
            if (mTypedArray.hasValue(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_labelBackground)) {
                ...
                mLabelBg = getResources().getDrawable(R.drawable.layoutk_label_default_background);
            }

            isSingleLine = mTypedArray.getBoolean(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_singleLine, false);
            isTextBold = mTypedArray.getBoolean(R.styleable.LayoutKLabelGroup_layoutKLabelGroup_isTextBold, false);
...
//            测试的数据
    ...
    private void measureSingleLine(int widthMeasureSpec, int heightMeasureSpec) {
        if (mLabelColumnCount > 0) {
            ...
            mLabelWidth = (widthSize - getPaddingLeft() - getPaddingRight() - mWordMargin * (mLabelColumnCount - 1)) / mLabelColumnCount;
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (mLabelColumnCount > 0) {
                int makeMeasureSpec = MeasureSpec.makeMeasureSpec(mLabelWidth, MeasureSpec.EXACTLY);
                measureChild(view, makeMeasureSpec, heightMeasureSpec);
            } else {
...
//            }
     ...
     * @param l
     */
    public <T> void setOnLabelClickListener(OnLabelClickListener<T> l) {
    ...
    public interface OnLabelClickListener<T> {
         ...
         * @param position 标签位置
         */
        void onLabelClick(TextView label, T data, int position);
    }
