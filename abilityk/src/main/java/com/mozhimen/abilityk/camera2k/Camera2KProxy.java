...
import com.sensetime.notable.handsample.opengl.SDKDetectUtils;
import com.sensetime.notable.render.TextureController;
...
public class Camera2KProxy {
    ...
    public Camera2KProxy(Activity activity, TextureController mController) {
        mActivity = activity;
    ...
    public void openCamera(int width, int height) {
        ...
        try {
            ...
            _controller.setDataSize(mPreviewSize.getHeight(), mPreviewSize.getWidth());
            // 打开摄像头
            mCameraManager.openCamera(Integer.toString(mCameraId), mStateCallback, mBackgroundHandler);
        } catch (CameraAccessException | IllegalArgumentException e) {
            Log.e("CameraV2",e.getMessage());
            Log.e("CameraV2","摄像头打开失败！");
            mActivity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(mActivity, "请检查摄像头", Toast.LENGTH_SHORT).show();
                }
            });
            //mActivity.finish();
        }
    ...
    private void initPreviewRequest() {
        try {
            ...
            mPreviewSurface = new Surface(_controller.getTexture());
            ...
            mPreviewRequestBuilder.addTarget(mImageReader.getSurface()); // 设置预览输出的 Surface
            mCameraDevice.createCaptureSession(Arrays.asList(mPreviewSurface, mImageReader.getSurface()),
                    new CameraCaptureSession.StateCallback() {
                        ...
                        public void onConfigured(@NonNull CameraCaptureSession session) {
                            ...
                            try {
                                session.setRepeatingRequest(mPreviewRequestBuilder.build(), new CameraCaptureSession.CaptureCallback() {
                                    @Override
