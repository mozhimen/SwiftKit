...
public class AppDownloadSerialQueue extends DownloadListener2 implements Runnable {
    ...
    public AppDownloadSerialQueue() {
        this(null);
    }

    AppDownloadSerialQueue(DownloadListener listener, ArrayList<DownloadTask> taskList) {
    ...
    public AppDownloadSerialQueue(DownloadListener listener) {
    ...
    public synchronized void remove(DownloadTask task) {
        ...
        while (iterator.hasNext()) {
            ...
            if (next.equals(task)) {
                StatusUtil.Status status = StatusUtil.getStatus(next);
