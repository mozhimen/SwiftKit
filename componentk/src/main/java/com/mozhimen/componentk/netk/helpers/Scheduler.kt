...
import com.mozhimen.componentk.netk.annors._CacheStrategy
import com.mozhimen.componentk.netk.commons.INetKCall
import com.mozhimen.componentk.netk.commons.INetKListener
        ...
        override fun enqueue(callback: INetKListener<T>) {
            ...
            if (_request.cacheStrategy == _CacheStrategy.CACHE_FIRST) {
                ExecutorK.execute(TAG, runnable = {
                    ...
                    if (cacheResponseK.data != null) {
                        //抛到主线程
                        RefHandler(this@Scheduler).sendMsgAtFrontOfQueue {
                            callback.onSuccess(
                                cacheResponseK
                            )
                        }
                        LogK.dt(TAG, "enqueue cacheStrategy key ${_request.getCacheKey()}")
                    }
                })
            }
            ...
            _delegate.enqueue(object : INetKListener<T> {
                override fun onSuccess(response: NetKResponse<T>) {
                    ...
                    LogK.dt(TAG, "enqueue save cache cacheKey ${_request.getCacheKey()}")
                }
