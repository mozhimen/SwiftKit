 ...
 * 实现自动翻页的ViewPager
 */
public class BannerMKViewPager extends ViewPager {
     ...
     * @param context
     */
    public BannerMKViewPager(@NonNull Context context) {
        super(context);
    }

    public BannerMKViewPager(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public void setScrollerDuration(int duration) {
        try {
            ...
            scrollerField.set(this, new BannerMKScroller(getContext(), duration));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private int next() {
        ...
        if (nextPosition >= getAdapter().getCount()) {
            //获取第一个item的索引
            nextPosition = ((BannerMKAdapter) getAdapter()).getFirstItem();
        }
