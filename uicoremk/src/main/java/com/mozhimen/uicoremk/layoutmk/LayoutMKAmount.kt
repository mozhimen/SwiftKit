...
typealias LayoutMKAmountListener = (Int) -> Unit

class LayoutMKAmount @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null, defStyleAttr: Int = 0
) : LinearLayout(context, attrs, defStyleAttr), ILayoutMK {

    private var _layoutMKAmountListener: LayoutMKAmountListener? = null
    ...
    fun setAmountValueChangedListener(layoutMKAmountListener: LayoutMKAmountListener) {
        this._layoutMKAmountListener = layoutMKAmountListener
    }
    ...
    private fun applyAttrs() {
        ...
        decreaseBtn.setOnClickListener {
            ...
            _layoutMKAmountListener?.invoke(_amountValue)
        }
    ...
    private object AttrsParser {
        fun parseAttrs(context: Context, attrs: AttributeSet?, defStyleAttr: Int): Attrs {
            val array = context.obtainStyledAttributes(
                attrs, R.styleable.LayoutMKAmount, defStyleAttr, R.style.LayoutMKAmountStyle
            )

            val btnTextSize = array.getDimensionPixelSize(
                R.styleable.LayoutMKAmount_layoutMKAmount_btn_textSize, 14f.sp2px()
            )
            val btnTextColor = array.getColorStateList(
                R.styleable.LayoutMKAmount_layoutMKAmount_btn_textColor
            )
            val btnSize = array.getDimensionPixelSize(
                R.styleable.LayoutMKAmount_layoutMKAmount_btn_size, 20f.dp2px()
            )
            val margin = array.getDimensionPixelOffset(
                R.styleable.LayoutMKAmount_layoutMKAmount_margin, 0
            )
            val btnBackground = array.getColor(
                R.styleable.LayoutMKAmount_layoutMKAmount_btn_background, Color.parseColor("#eeeeee")
            )
            val numTextSize = array.getDimensionPixelSize(
                R.styleable.LayoutMKAmount_layoutMKAmount_num_textSize, 14f.sp2px()
            )
            val numTextColor = array.getColor(
                R.styleable.LayoutMKAmount_layoutMKAmount_num_textColor, Color.BLACK
            )
            val numSize = array.getDimensionPixelSize(
                R.styleable.LayoutMKAmount_layoutMKAmount_num_size, 20f.dp2px()
            )
            val numBackground = array.getColor(
                R.styleable.LayoutMKAmount_layoutMKAmount_num_background, Color.WHITE
            )
            val numDefaultValue = array.getInteger(
                R.styleable.LayoutMKAmount_layoutMKAmount_num_defaultValue, 1
            )
            val numMinValue = array.getInteger(
                R.styleable.LayoutMKAmount_layoutMKAmount_num_minValue, 1
            )
            val numMaxValue = array.getInteger(
                R.styleable.LayoutMKAmount_layoutMKAmount_num_maxValue, Int.MAX_VALUE
            )
