 ...
 * @Version 1.0
 */
class LayoutMKVideo : FrameLayout, TextureView.SurfaceTextureListener {

    private val TAG = "LayoutMKVideo>>>>>"
    var videoFileName = ""
    ...
    private fun init(attrs: AttributeSet?) {
        val typeArray = context.theme.obtainStyledAttributes(attrs, R.styleable.LayoutMKVideo, 0, 0)

        try {
            videoFileName = typeArray.getString(R.styleable.LayoutMKVideo_layoutMKVideo_pathOrUrl).toString()
            videoGravity = typeArray.getInt(R.styleable.LayoutMKVideo_layoutMKVideo_videoGravity, 2)
            videoIsLoop = typeArray.getBoolean(R.styleable.LayoutMKVideo_layoutMKVideo_isLoop, false)
        } finally {
            typeArray.recycle()
        }
