...
import com.mozhimen.basicsmk.utilmk.UtilMKBitmapBlur
 ...
 * @Version 1.0
 */
class LayoutMKBlurred : RelativeLayout {
    ...
    private val TAG = "LayoutMKBlurred>>>>>"
    ...
    private fun init(context: Context) {
        mContext = context
        LayoutInflater.from(context).inflate(R.layout.layoutmk_blurred, this)
        originImageView = findViewById(R.id.layoutmk_img_origin)
        blurredImageView = findViewById(R.id.layoutmk_img_blurred)
    }

    @RequiresApi(Build.VERSION_CODES.JELLY_BEAN_MR1)
    private fun initAttr(context: Context, attrs: AttributeSet) {
        val typedArray = context.obtainStyledAttributes(attrs, R.styleable.LayoutMKBlurred)
        val drawable = typedArray.getDrawable(R.styleable.LayoutMKBlurred_layoutMKBlurred_src)
        isMove = typedArray.getBoolean(R.styleable.LayoutMKBlurred_layoutMKBlurred_isMove, false)
        isDisableBlurred = typedArray.getBoolean(R.styleable.LayoutMKBlurred_layoutMKBlurred_isDisableBlurred, false)

        typedArray.recycle()

        //模糊图片
        drawable?.let {
            mOriginBitmap = UtilMKBitmap.drawable2Bitmap(drawable)
            mBlurredBitmap = UtilMKBitmapBlur.blur(context, mOriginBitmap)
        }
    ...
    fun setBlurredImageView(blurredBitmap: Bitmap?) {
        blurredBitmap?.let {
            ...
            mBlurredBitmap = UtilMKBitmapBlur.blur(mContext, blurredBitmap)
            setImageView()
    ...
    fun setBlurredImageView(blurDrawable: Drawable?) {
        blurDrawable?.let {
            mOriginBitmap = UtilMKBitmap.drawable2Bitmap(blurDrawable)
            mBlurredBitmap = UtilMKBitmapBlur.blur(mContext, mOriginBitmap)
            setImageView()
            setMove(mContext, isMove)
        }
    }
