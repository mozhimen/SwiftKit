...
import com.mozhimen.uicoremk.btnmk.BtnMKIconFont
 ...
 * @Version 1.0
 */
class NavMKBar @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0
) : RelativeLayout(context, attrs, defStyleAttr) {

    private var _titleView: TextMKIconFont? = null
    private var _subTitleView: TextMKIconFont? = null
    ...
    //属性解析获得对象
    private val _attrs: NavMKAttrs by lazy { NavMKAttrsParser.parseNavAttrs(context, attrs, defStyleAttr) }
    ...
    fun setNavListener(listener: OnClickListener) {
        if (!TextUtils.isEmpty(_attrs.iconStr)) {
            val navBackView = addLeftTextButton(_attrs.iconStr!!, R.id.navmk_icon_left_back)
    ...
    fun addLeftTextButton(@StringRes stringRes: Int, viewId: Int): Button {
        return addLeftTextButton(UtilMKRes.getString(stringRes), viewId)
    }
    ...
    private fun ensureTitleView() {
        if (_titleView == null) {
            _titleView = TextMKIconFont(context, null)
            _titleView?.apply {
    ...
    private fun ensureSubTitleView() {
        if (_subTitleView == null) {
            _subTitleView = TextMKIconFont(context, null)
            _subTitleView?.apply {
                gravity = Gravity.CENTER
                isSingleLine = true
                ellipsize = TextUtils.TruncateAt.END
    ...
    private fun ensureTitleContainer() {
        if (_titleContainer == null) {
            ...
            _titleContainer?.apply {
                ...
                this@NavMKBar.addView(_titleContainer, params)
            }
        }
    }
    ...
    private fun generateTextButton(): Button {
        val button = BtnMKIconFont(context)
