package com.mozhimen.uicoremk.datamk.helpers
...
/**
 * @ClassName DataMKRecycler
 * @Description TODO
 * @Author Kolin Zhao
 * @Date 2021/8/31 16:14
 * @Version 1.0
 */
class DataMKAdapter(context: Context) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    private var mContext: Context = context
    private var mInflater: LayoutInflater? = null
    private var dataSets = ArrayList<DataMKItem<*, out RecyclerView.ViewHolder>>()
    private var typeArrays = SparseArray<DataMKItem<*, out RecyclerView.ViewHolder>>()
     ...
     * 在指定位上添加item
     */
    fun addItemAt(index: Int, item: DataMKItem<*, out RecyclerView.ViewHolder>, notify: Boolean) {
        if (index > 0) {
     ...
     * 添加items集合
     */
    fun addItems(items: List<DataMKItem<*, out RecyclerView.ViewHolder>>, notify: Boolean) {
     ...
     * 从指定位上移除item
     */
    fun removeItemAt(index: Int): DataMKItem<*, out RecyclerView.ViewHolder>? {
     ...
     * 移除item
     */
    fun removeItem(dataMKItem: DataMKItem<*, out RecyclerView.ViewHolder>) {
    ...
    private fun createViewHolderInternal(
        javaClass: Class<DataMKItem<*, out RecyclerView.ViewHolder>>,
        view: View
    ): RecyclerView.ViewHolder {
        //得到该item的父类型,即为DataMKItem.class. class也是type的一个子类
            ...
            arguments.forEach {
                if (it is Class<*> && RecyclerView.ViewHolder::class.java.isAssignableFrom(
                    ...
                    try {
                        ...
                        //这里需要 try-catch, 如果直接在DataMKItem子类上标记 RecyclerView.ViewHolder. 抽象类是不允许反射的
    ...
    fun refreshItem(dataMKItem: DataMKItem<*, out RecyclerView.ViewHolder>) {
        val indexOf = dataSets.indexOf(dataMKItem)
        notifyItemChanged(indexOf)
    }

    fun getItem(position: Int): DataMKItem<*, RecyclerView.ViewHolder>? {
        if (position < 0 || position >= dataSets.size) {
            return null
        }
        return dataSets[position] as DataMKItem<*, RecyclerView.ViewHolder>
    }
