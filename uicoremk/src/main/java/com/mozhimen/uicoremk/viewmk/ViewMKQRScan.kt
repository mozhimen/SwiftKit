...
class ViewMKQRScan : View {
    ...
    fun init(attrs: AttributeSet?) {
        ...
        val typedArray: TypedArray = context.obtainStyledAttributes(attrs, R.styleable.ViewMKQRScan)
    ...
    private fun initCustomAttr(attr: Int, typedArray: TypedArray) {
        when (attr) {
            R.styleable.ViewMKQRScan_viewMKQRScan_customScanLineDrawable -> {
                mCustomScanLineDrawable = typedArray.getDrawable(attr)
            }
            R.styleable.ViewMKQRScan_viewMKQRScan_borderSize -> {
                mBorderSize = typedArray.getDimensionPixelSize(attr, mBorderSize)
            }
            R.styleable.ViewMKQRScan_viewMKQRScan_borderColor -> {
                mBorderColor = typedArray.getColor(attr, mBorderColor)
            }
            R.styleable.ViewMKQRScan_viewMKQRScan_animTime -> {
                mAnimTime = typedArray.getInteger(attr, mAnimTime)
            }
            R.styleable.ViewMKQRScan_viewMKQRScan_isScanLineReverse -> {
