...
object PermissionMK {
    ...
    fun initPermissions(
        ...
        permissionMKCallback: PermissionMKCallback
    ) {
        ...
        if (permissions.isNotEmpty()) {
            if (!checkPermissions(activity, *permissions)) {
                requestPermissions(activity, *permissions) { allGranted, deniedList ->
                    permissionMKCallback(allGranted, deniedList)
                }
            } else {
                permissionMKCallback(true, emptyList())
            }
        } else {
