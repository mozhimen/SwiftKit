...
class VideoLayout : FrameLayout, TextureView.SurfaceTextureListener {
    ...
    private fun init(attrs: AttributeSet?) {
        val typeArray = context.theme.obtainStyledAttributes(attrs, R.styleable.VideoLayout, 0, 0)

        try {
            videoFileName = typeArray.getString(R.styleable.VideoLayout_path_or_url).toString()
            videoGravity = typeArray.getInt(R.styleable.VideoLayout_video_gravity, 2)
            videoIsLoop = typeArray.getBoolean(R.styleable.VideoLayout_loop, false)
        } finally {
            typeArray.recycle()
        }
    ...
    override fun onSurfaceTextureAvailable(surface: SurfaceTexture, width: Int, height: Int) {
        try {
            ...
            e.message?.let { LogUtil.e("Video: $mTag", it) }
    ...
    fun onVideoLayoutDestroy() {
        mMediaPlayer?.let {
            try {
                ...
                LogUtil.e(mTag, "onVideoDestroy: ${e.message}")
            } catch (e: Exception) {
                LogUtil.e(mTag, "onVideoDestroy: ${e.message}")
            }
        }
    ...
    private fun changeVideo(onCompletionListener: MediaPlayer.OnCompletionListener) {
        try {
            ...
            e.message?.let { LogUtil.e("$mTag: Video", it) }
        } catch (e: SecurityException) {
            e.message?.let { LogUtil.e("$mTag: Video", it) }
        } catch (e: IllegalStateException) {
            e.message?.let { LogUtil.e("$mTag: Video", it) }
